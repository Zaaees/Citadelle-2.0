# Configurer l'encodage par défaut en UTF-8
$OutputEncoding = [System.Text.Encoding]::UTF8

# Fonction pour convertir en UTF-8
function ConvertFrom-Utf8([string]$text) {
    $bytes = [System.Text.Encoding]::GetEncoding(850).GetBytes($text)
    return [System.Text.Encoding]::UTF8.GetString($bytes)
}

# Récupérer les 5 derniers commits avec encodage UTF-8
$commits = git log -n 5 --pretty=format:"%h - %s (%cr)" | ForEach-Object { ConvertFrom-Utf8($_) } | Out-String
...
# Variable pour suivre l'index actuel dans l'historique des commits
$script:historyIndex = -1

# Extraire les messages de commit de l'historique avec encodage UTF-8
$commitMessages = @(git log -n 5 --pretty=format:"%s" | ForEach-Object { ConvertFrom-Utf8($_) })

# Gestionnaire d'événement pour les touches dans la zone de texte
$textBox.Add_KeyDown({
    param($sender, $e)
    
    # Flèche haut - naviguer vers les commits plus anciens
    if ($e.KeyCode -eq [System.Windows.Forms.Keys]::Up) {
        $e.SuppressKeyPress = $true  # Empêcher le comportement par défaut
        
        if ($script:historyIndex -lt ($commitMessages.Count - 1)) {
            $script:historyIndex++
            $textBox.Text = $commitMessages[$script:historyIndex]
            $textBox.SelectionStart = $textBox.Text.Length
        }
    }
    
    # Flèche bas - naviguer vers les commits plus récents
    elseif ($e.KeyCode -eq [System.Windows.Forms.Keys]::Down) {
        ...
        elseif ($script:historyIndex -eq 0) {
            $script:historyIndex = -1
            $textBox.Text = ""
        }
    }
